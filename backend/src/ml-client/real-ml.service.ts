...
export class RealMlService implements MlPredictor {
  ...
  private base() {
    return this.config.getOrThrow<string>('ML_SERVICE_URL');
  }
  ...
  private validate(data: any): PredictionResult {
    if (typeof data?.risk_score !== 'number' || !data?.verdict) {
      this.logger.warn(
        `ML service returned a malformed response: ${JSON.stringify(data)}`,
      );
    ...
    return {
      ...
      // Keep the source returned by the Python ML service.
      // This should be "url_xgboost_v3" for URL predictions.
  ...
  async predictUrl(url: string): Promise<PredictionResult> {
    try {
      ...
      this.logger.error(
        `URL prediction failed: ${err.message}`,
      );

      return this.unavailable(
        'ML service unavailable for URL check',
      );
    }
  }
  ...
  async predictText(text: string): Promise<PredictionResult> {
    try {
      ...
      this.logger.error(
        `Text prediction failed: ${err.message}`,
      );

      return this.unavailable(
        'ML service unavailable for text check',
      );
    }
  }
  ...
  async predictImage(form: FormData): Promise<PredictionResult> {
    try {
      ...
      this.logger.error(
        `Image prediction failed: ${err.message}`,
      );

      return this.unavailable(
        'ML service unavailable for image check',
      );
    }
  }
}